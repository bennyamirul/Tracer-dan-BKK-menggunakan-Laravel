...
<div class="container-xxl py-5">
    <div class="card mb-5 mb-xl-10">
        <div class="card-body py-5">
            <div class="d-flex justify-content-between align-items-center">
                <ul class="nav nav-custom nav-tabs nav-line-tabs nav-line-tabs-2x border-0 fs-5 fw-bolder flex-grow-1" role="tablist">
                    ...
                    @if(auth()->user()->role === 'admin_sekolah' && $user->role === 'pelamar_alumni')
                    <li class="nav-item" role="presentation">
                ...
                <a href="{{ route('admin.profile-user.show', $user->id) }}" class="btn btn-light ms-3">
    ...
    <div class="tab-content">
        <div class="tab-pane fade show active" id="tab-edit-data-diri" role="tabpanel">
            <form action="{{ route('admin.profile-user.update', $user->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                            ...
                            <div class="card-body d-flex flex-column align-items-center">
                                ...
                                <div class="mt-4 w-100">
                                    ...
                                    <select name="status" class="form-select form-select-solid">
                                        ...
                                        <option value="inactive" {{ old('status', $user->status ?? 'inactive') == 'inactive' ? 'selected' : '' }}>Nonaktif</option>
                                    </select>
                                    @error('status')
                                    <div class="text-danger small">{{ $message }}</div>
                                    @enderror
                    ...
                    <div class="col-md-8">
                        <div class="card h-100">
                            ...
                            <div class="card-body">
                                ...
                                @if($errors->any() && (old('section') === 'user' || !old('section')))
                                <div class="alert alert-danger alert-dismissible fade show" role="alert">
                                    <strong>Terjadi kesalahan!</strong> Silakan periksa kembali form di bawah.
                                    <ul class="mb-0 mt-2">
        ...
        <div class="tab-pane fade" id="tab-edit-tracer" role="tabpanel">
            @if(session('status') && session('activeTab') === 'tracer')
            ...
            @if($errors->any() && old('section') === 'tracer')
            ...
            <form action="{{ route('admin.profile-user.update', $user->id) }}" method="POST">
                @csrf
                @method('PUT')
        ...
        <div class="tab-pane fade" id="tab-edit-berkas" role="tabpanel">
            ...
            @if($errors->any() && old('section') === 'berkas')
            <div class="alert alert-danger alert-dismissible fade show mb-5" role="alert">
                <strong>Terjadi kesalahan!</strong> Silakan periksa kembali form di bawah.
                <ul class="mb-0 mt-2">
                    @foreach ($errors->all() as $error)
                        ...
                        <div class="card-body pt-5">
                            ...
                            <div class="d-flex align-items-center justify-content-between bg-light-primary rounded p-3 mb-3">
                                ...
                                <div class="d-flex gap-2">
                                    ...
                                    <form action="{{ route('admin.profile-user.berkas.delete', [$user->id, $item->id]) }}" method="POST" class="d-inline delete-berkas-form">
                                        @csrf
                                        @method('DELETE')
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        var hasErrors = '{{ $errors->any() ? "yes" : "no" }}';
        var errorSection = '{{ old("section") }}';

        // Jika ada error validasi, set tab sesuai section yang error
        if (hasErrors === 'yes' && errorSection) {
            if (errorSection === 'tracer') {
                activeTab = 'tracer';
            } else if (errorSection === 'berkas') {
                activeTab = 'berkas';
            } else {
                activeTab = 'data';
            }
        ...
        if (activeTab === 'data') {
            var dataTab = document.querySelector('a[href="#tab-edit-data-diri"]');
            if (dataTab) {
